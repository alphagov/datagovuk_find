...
module SolrDatasetsHelper
  ...
  def get_datafiles
    @datafiles = @dataset_dict["resources"]
    @datalinks = []
    @supporting_docs = []
  
    @datafiles.each do |datafile|
      datafile["resource-type"] == "supporting-document" ? @supporting_docs << datafile : @datalinks << datafile
      SolrDatafile.new(datafile)
    end
  end
